"""
DOC/DOCX to PDF 변환 유틸리티

Microsoft Word COM 자동화를 사용하여 DOC/DOCX 파일을 PDF로 변환합니다.
Word가 없는 경우 LibreOffice를 폴백으로 사용합니다.

사용법:
    python doc2pdf.py input.docx                    # 단일 파일 변환
    python doc2pdf.py input.docx -o output.pdf       # 출력 경로 지정
    python doc2pdf.py ./docs/                        # 폴더 내 모든 DOC/DOCX 일괄 변환
...
def convert_with_word(input_path: str, output_path: str) -> bool:
    """Microsoft Word COM 자동화를 사용하여 PDF로 변환한다."""
    try:
...
def convert_file(input_path: str, output_path: str, engine: str = "auto") -> bool:
    ...
    if engine == "word":
        ...
        if convert_with_word(input_path, output_path):
            return True
        return convert_with_libreoffice(input_path, output_path)


def collect_doc_files(path: str) -> list[Path]:
    """경로에서 DOC/DOCX 파일 목록을 수집한다."""
    p = Path(path).resolve()
    if p.is_file():
        ...
        files = sorted(
            f for f in p.rglob("*")
            if f.suffix.lower() in DOC_EXTENSIONS and not f.name.startswith("~$")
        )
        return files
    else:
...
def main():
    parser = argparse.ArgumentParser(
        description="DOC/DOCX 파일을 PDF로 변환합니다.",
    )
    parser.add_argument(
        "input",
        nargs="+",
        help="변환할 DOC/DOCX 파일 또는 폴더 경로",
    )
    parser.add_argument(
        "-o", "--output",
    ...
    parser.add_argument(
        ...
        default="auto",
        help="변환 엔진 선택 (기본값: auto - Word 우선, LibreOffice 폴백)",
    )
    ...
    files = collect_doc_files(input_path)
    if not files:
        print("변환할 파일이 없습니다.")
        sys.exit(1)
