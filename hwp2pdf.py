"""
HWP/HWPX to PDF 변환 유틸리티

한컴오피스(한글) COM 자동화를 사용하여 HWP/HWPX 파일을 PDF로 변환합니다.
한컴오피스가 없는 경우 LibreOffice를 폴백으로 사용합니다.
...
def convert_with_hancom(input_path: str, output_path: str) -> bool:
    """한컴오피스 COM 자동화를 사용하여 PDF로 변환한다."""
    try:
...
def collect_hwp_files(path: str) -> list[Path]:
    ...
    if p.is_file():
        ...
        files = sorted(
            f for f in p.rglob("*") if f.suffix.lower() in HWP_EXTENSIONS
        )
        return files
    else:
...
def main():
    parser = argparse.ArgumentParser(
        description="HWP/HWPX 파일을 PDF로 변환합니다.",
    )
    parser.add_argument(
        "input",
        nargs="+",
